...
class AdapterPhones: RecyclerView.Adapter<AdapterPhones.ItemPhonesViewHolder>(){
    ...
    class ItemPhonesViewHolder(val bindingPhone: ItemPhonesBinding) :
        RecyclerView.ViewHolder(bindingPhone.root) {
        fun bind(phoneVista: PhoneEntity) {
            ...
            bindingPhone.cardViewItem.setOnClickListener {

                val bundle = Bundle()
                bundle.putString("id", phoneVista.id.toString())

                Navigation.findNavController(bindingPhone.root)
                    .navigate(R.id.action_listPhoneFragment_to_detallePhoneFragment, bundle)
